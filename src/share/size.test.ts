...
import { defaultScenario, defaultSportsInput, editorScenarioGame, MAX_STRATEGIES, newCustomInstance, newStrategyInstance, sportsScenarioGame, validateScenario, type ScenarioConfig } from "../scenario";
...
describe("link size budget", () => {
  ...
  it(`the typical scenario on a 12-OUTCOME game (fractions and labels) is still under ${TYPICAL_URL_BUDGET} characters, and round-trips`, () => {
    const rows = Array.from({ length: 12 }, (_, i) => ({ prob: i < 11 ? "1/13" : "2/13", value: 5 * i, label: `Prize ${i + 1}` }));
    const s = { ...typicalScenario(), game: editorScenarioGame({ mode: "ticket", price: 25, rows }) };
    expect(validateScenario(s)).toEqual({});
    const enc = encodeScenarioLink(s, BASE);
    if (!enc.ok) throw new Error(enc.message);
    console.log(`[size] typical scenario, 12-outcome game: fragment ${enc.fragment.length} chars/bytes + ${ORIGIN_ALLOWANCE} address allowance = ${enc.urlLength} (budget ${TYPICAL_URL_BUDGET})`);
    expect(enc.urlLength).toBeLessThan(TYPICAL_URL_BUDGET);
    const dec = decodeScenarioLink(enc.fragment);
    expect(dec.kind === "loaded" && content(dec.scenario)).toEqual(content(s));
  });
